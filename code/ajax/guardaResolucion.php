...
//actualizamos la transferencia a TERMINADA				
		$sql="UPDATE ec_transferencias SET id_estado=6,observaciones=CONCAT(observaciones,'\n','Resolución resuelta por ','$usuario_resolucion'), ultima_actualizacion=DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i:%s') WHERE id_transferencia=$id";		
		$res=mysql_query($sql);	  
		if(!$res){
			echo "Error en:\n$sql\n\nDescripcion:\n".mysql_error();
	...
	mysql_query("BEGIN");//marcamos el inicion de la transacción
...
/*implementación Oscar 10.04.2019 para la resolución de transferencias*/
	...
	//realizamos busqueda por coincidencia
		$sql="SELECT 
			...
			WHERE sp.id_sucursal={$user_sucursal} AND sp.estado_suc=1
			AND (";
