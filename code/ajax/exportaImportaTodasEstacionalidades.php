...
//extraemos las estacionalidades altas	
	...
	$eje_2=mysql_query($sql) or die("Error al consultar las estacionalides 1 !!!".$sql."\n\n".mysql_error());

//generamos encabezados para cada estacionalidad
	$sql="SELECT 'venta_mas_alta','promedio','id_registro','id_estacionalidad','id_producto','nombre','maximo' 
...
//formamos la cabecera 
	...
	//echo utf8_decode($dat[0].','.$dat[1].','.$dat[2].','.$dat[3].',');

	echo utf8_decode('Datos del producto,'.','.','.',');
	//while($dat_enc=mysql_fetch_row($eje_2)){
...
//		$sucs.=$dat_enc[4]."|";//guardamos las sucursales
	...
	for($i=0;$i<$num_estac;$i++){
		...
		echo $dat_enc[0].','.$dat_enc[1].','.$dat_enc[2].','.$dat_enc[3].','.$dat_enc[4].','.$dat_enc[5].','.$dat_enc[6];
		if($i<($num_estac-1)){
			echo utf8_decode(',');
		}else{
			echo utf8_decode("\n");
...
//die($sql);
		...
		for($j=0;$j<mysql_num_rows($eje_3);$j++){
			...
			echo utf8_decode($dat_est_pro[0].','.$dat_est_pro[1].','.$dat_est_pro[2].','.$dat_est_pro[4].','.$dat_est_pro[3].','.$dat_est_pro[5].','.$dat_est_pro[6]);
