	...
	//descomprimimos registros
		...
		for($i=0;$i<sizeof($arr);$i++){
			...
			if($arr1[0]!=""){
				...
				if($arr1[4]==0){	
					$sql="INSERT INTO ec_proveedor_producto VALUES(null,$arr1[0],$id,$prec_caja,$arr1[2],$arr1[1],'$arr1[3]',null,null)";
				}else{
					$sql="UPDATE ec_proveedor_producto SET precio=$prec_caja,presentacion_caja=$arr1[2],precio_pieza=$arr1[1],
					clave_proveedor='$arr1[3]',ultima_actualizacion=now() WHERE id_proveedor_producto=$arr1[4]";
				}
				//echo $sql;
				$eje=mysql_query($sql);
				if(!$eje){
...
//guardamos el pedido
	if($fl==10){
		mysql_query("BEGIN");//marcamos inicio de transacción
		$ids_asignados="";
		//$arr=str_replace("Â", "", $arr);//implementacion oscar por fallo de Â
