...
FROM(
	...
	/*3*IF(ax.invCompras IS NULL,0,ax.invCompras) AS invCompras_1,inventario de compras se arrastra desde ax1*/
    ...
    /*12*/'vtasAnt',/*- este no se usa...*/
    /*13*/ROUND(IF((ax.pedidoPzas/*-ax.cantPedRecib*/)<0,0,(ax.pedidoPzas/*-ax.cantPedRecib/*$factor*/))) as pedidoPiezas,/*-ax.ventas; lo multiplicamos por -1 para cconvertirlo en positivo*/
    /*14*/prov.nombre_comercial,
    /*15*/ax.precioMinimo,/*este no se usa*/
    /*16*/ax.es_nuevo,/*marca si es nuevo el producto(cuando no tiene movimientos de almacen)*/
	/*17*/ax.cantPedAnt,
    ...
    /**/   	ax1.ventasTotales,
			ax1.cantPedAnt,
			ax1.cantPedRecib,
			IF('$fechas'='n/a',/*si las fechas no aplican*/
	...
	/**/
        ...
        /**/	ax2_3.invFinal,
				...
				ax2_3.cantPedRecib,
		/**/
				ax2_3.cantPedAnteriormente/*-(SUM(IF(rd.id_oc_recepcion_detalle IS NOT NULL,rd.piezas_recibidas,0))) */ as cantPedAnt,
				ax2_3.observaciones,
        	...
        	/**/	ax2_2.invFinal,
					ax2_2.es_nuevo,
			/*************Se deshabilitan las devoluciones porque al momento de hacer una devolucion la cantidad del detalle de la venta se actualiza de 
			acuerdo a la devolución; entonces al incluirlas se descuenta el doble de devoluciones a las ventas***********/
					ax2_2.ventas/*ax2_2.devoluciones*)-IF(ax2_2.devoluciones_paquete IS NULL,0,0/*ax2_2.devoluciones_paquete))*/ AS ventasTotales,
	/*Sumamos todos los detalles de ordenes de compra por producto*/
					SUM(IF(ocd.id_oc_detalle IS NULL OR oc_1.id_estatus_oc>=4,0,ocd.cantidad)) AS cantPedAnteriormente,
			/*implementacion Oscar 19.07.2019 para calcular lo que ya se recibio*/			
			...
			/**/	/**/    ax2.invFinal,
							ax2.es_nuevo,
							$ventas
					/*Implementación Oscar 02.04.2019 para sumar las ventas de los productos */
							...
							ax2.invFinalMatriz
					/*Fin de cambio Oscar 02.04.2019*/
			/**/	/**/	/*IF(ped.id_pedido IS NULL OR pd.id_pedido_detalle IS NULL,0,SUM(pd.cantidad)) AS ventas*/
			/**/	/**/	/*IF(SUM(pd.cantidad) IS NULL,0,SUM(pd.cantidad)) AS ventas*/
    	    /**/    /**/    FROM(
    	    /**/    /**/   	/**/SELECT/*calculamos el inventario actual del año corriendo*/
        	/**/    /**/   	/**/	ax3.id_productos,/*id de producto*/
        	/**/    /**/	/**/	ax3.ordenLista,/*orden de lista*/
			/**/	/**/	/**/	ax3.nombre,	/*nombre del producto*/
			/**/	/**/	/**/	ax3.resurtible,/*resurtible/no resurtible*/
			/**/	/**/	/**/	ax3.precioMinimo,/**/
			...
			/**/	/**/	/**/	/**/	/*IF(ma3.id_movimiento_almacen IS NULL,0,(SUM(md3.cantidad*tm3.afecta)*-1)) AS ventasAntes/*IF(SUM(pd.cantidad) IS NULL,0,SUM(									pd.cantidad)) AS ventas*/
			/**/	/**/	/**/	/**/	IF((pr_prod.id_producto)IS NULL OR pr_prod.precio<=0,'1.00',MIN(pr_prod.precio_pieza)) as precioMinimo,
			/**/	/**/	/**/	/**/	p.observaciones
			/**/	/**/	/**/	/**/	FROM ec_productos p
			...
			/**/	/**/	/**/	/**/	WHERE 1 $filtro1 $f_resurtimiento
			/**/	/**/	/**/	/**/	AND p.es_maquilado=0/*Implementación Oscar 01.04.2019 para excluir productos maquilados*/
			/**/	/**/	/**/	/**/	$status_producto
        	...
        	/**/   	/**/		GROUP BY ax2.id_productos
						...
						LEFT JOIN ec_ordenes_compra oc_1 ON ocd.id_orden_compra=oc_1.id_orden_compra /*AND oc_1.id_estatus_oc<=3/*menos o igaul a 3 (antes de ser finalizadala la orden de compra)*/
		...
		/**/	LEFT JOIN ec_movimiento_almacen ma1 ON md1.id_movimiento = ma1.id_movimiento_almacen/*$f_compras*/
		/**/	LEFT JOIN ec_tipos_movimiento tm1 ON ma1.id_tipo_movimiento = tm1.id_tipo_movimiento /*AND tm1.id_tipo_movimiento=1 */
        /**/   	LEFT JOIN ec_almacen alm1 ON ma1.id_almacen=alm1.id_almacen /*AND alm1.es_externo=0*/
        		WHERE 1
        		GROUP BY ax1.id_productos
        /**/)ax
			LEFT JOIN ec_proveedor_producto pr ON ax.id_productos=pr.id_producto AND pr.precio_pieza in(ax.precioMinimo)
        	LEFT JOIN ec_proveedor prov ON pr.id_proveedor=prov.id_proveedor 
			WHERE ax.id_productos>1$provs $pendientes $nuevos_prod 
