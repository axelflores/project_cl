<?php
	...
	$sql="SELECT 
   			...
   			ax.actual,
   			ax.suma_actual,
   			eh.suma_estacionalidades
   		FROM(
   			SELECT
   				c.nombre,
    			COUNT(ep.id_estacionalidad_producto)as actual,
    			c.id_categoria,
		...
		WHERE eh.id_sucursal=$id_sucursal";

	$eje=mysql_query($sql)or die("Error al consultar histórico y esatcionalidad actual!!!".mysql_error());
	echo '<button style="padding:10px;border-radius:50%;background:red;position:absolute;right:0;top:0px;"'.
	'onclick="document.getElementById(\'simula_tooltip\').style.display=\'none\';">X</button>';
//armamos la tabla
	echo '<center><table width="90%">';
		echo '<tr>';
			echo '<th width="20%">FAMILIA</th>';
			echo '<th width="20%">CANT ANT</th>';
			echo '<th width="20%">CANT ACT</th>';
			echo '<th width="20%">SUMA ANT</th>';
			echo '<th width="20%">SUMA ACT</th>';
		echo '</tr>';
	...
	echo '<tr '.$color_fila_tooltip.'><td></td><td align="right">'.$suma_1.'</td><td align="right">'.$suma_2.'</td></td><td align="right">'.$suma_3.'</td></td><td align="right">'.$suma_4.'</td><tr>';
