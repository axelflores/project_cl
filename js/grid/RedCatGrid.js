...
/*Implementación de Oscar para desplazarse con las teclas*/
function validaEventoTeclado(e,obj){
	var tecla_grid=e.keyCode;
//sacamos el nombre de la tabla
	var aux_nom_tba=$(obj).attr("id").split("_");
	var nom_tba=aux_nom_tba[0];
	//alert(nom_tba);
	//alert($(obj).attr("id"));
//tecla arriba
	if(tecla_grid==38){
...
//tecla abajo/eneter
	if(tecla_grid==40||tecla_grid==13){
...
var posgen;
var ultfilaed;
function CargaGrid(NomId,tabla_list){//alert(tabla_list);
	var tipos = new Array();
	var mods = new Array();
	var lons = new Array();
	var mask = new Array();
	...
	//alert(DatosFile);//prueba oscar 26.02.2018
	...
	for(var j=0;j<Tds.length;j++)
	{
		...
		reqs[j] =Tds[j].htmldebase?Tds[j].htmldebase:Tds[j].getAttribute("requerido");
		...
		else{//aqui se grega la variable tabla_list en la función reordenamiento de la ceabecera del grid Oscar 24.05.2018 
			cab[j]='<input type="button" value="'+texto+'" style="width:'+(largo)+'px; height:20px;" class="buttonHeader" id="H'+NomId+j+'" onclick="ordena('+(j+1)+', \''+NomId+'\',\''+tabla_list+'\');" onkeydown="TeclasEspeciales(event, this, \''+NomId+'_'+(j+1)+'_NaN\');"/>';
		}
	...
	/*Agregamos la columna para eliminar en caso de ser necesario*/
	...
	if(conScroll == 'S')
	{
		datos='<div id="div_'+NomId+'_overflow_Y" style="overflow:auto; height:'+(AltoGral-altomenos)+'px; padding:0">';
		var onlyDiv='<div id="div_'+NomId+'_overflow_Y" style="overflow:auto; height:'+(AltoGral-altomenos)+'px; padding:0"></div>';
	}
	datos+='<table cellpadding="'+cellP+'" border="'+borde+'" bordercolor="'+brcolor+'" cellspacing="'+cellS+'" id="Body_'+NomId+'" >';
	...
	{	
		for(k=1;k<Trs.length;k++)
		{
			...
			for(l=0;l<Tds.length;l++)
			{
				...
				if(bloqueoDatos!="S")
					datos+=' onclick="EditarValor(event,this);"';
	...
	else
	{
		...
		if(arrF[0] == 'exito')
		{
			...
			//implementación de Oscar para poner transferencias impreas en verde 25.05.2018*/
				var color_transfer='';
				if(tabla_list=='ZWNfdHJhbnNmZXJlbmNpYXM='&& axF[6]==1){
					color_transfer='style="background:rgba(0,225,0,.5);"';
				}
			//Fin de cambio 24.05.2018			
			
		/*implementacion Oscar 01.07.2019 para cambiar el color de las filas en el listado de cortes de caja */
				if(tabla_list=='ZWNfc2VzaW9uX2NhamE='){
				//verificado y con diferencia
					if(axF[10]!='0' && axF[9]=='verificado'){
						color_transfer='style="background:red;color:white;"';
					}
				//verificado y sin diferencia
					else if(axF[10]=='0' && axF[9]=='verificado'){
						color_transfer='style="background:rgba(0,225,0,.5);"';
					}				
				}
			/*fin de cambio Oscar 01.07.2019*/

			if(NomId=='usuariosAccesos'){
				if(axF[11]==0){
						color_transfer='style="background:rgba(225,0,0,.5);"';
				}
			}
		/*implementacion Oscar 18.07.2019 para colorear en rojo el listado de remisiones cuando hay diferencia*/
			//alert(tabla_list);
			if(tabla_list=='ZWNfb2NfcmVjZXBjaW9u'){
				//alert(axF[3]+'|'+axF[4]+"----"+axF[5]+'|'+axF[6]);
				if(axF[9]=='Pendiente de Precio/ Recibir '){//axF[3]!=axF[4] || axF[5]!=axF[6] || (axF[3]==0||axF[4]==0||axF[5]==0||axF[6]==0)
					color_transfer='style="background:rgba(225,0,0,.5);"';
				}	
				if(axF[9]!='Pendiente de Pagar'){//axF[3]==axF[4] && axF[5]==axF[6] && 
					color_transfer='style="background:rgba(0,225,0,.5);"';
				}	
			}
		/*fin de cambio Oscar 18.07.2019*/

				datos+='<tr class="NormalCell" '+color_transfer+' height="'+alto+'" onmouseover="selfil(this);" onmouseout="dselfil(this);" id="'+NomId+'_Fila'+(k-1);
		/*implementación oscar 20.03.2019 para meter el tooltip*/
				if(tabla_list=='ZWNfdHJhbnNmZXJlbmNpYXM='){
					datos+=' title="'+axF[7]+'"';
				}			
				datos+='">';	
		/*Fin de Cambio Oscar 20.03.2019*/				
				datos+='<td width="21px" class="Contador" align="center">'+k+'</td>';
			//alert(datos);
	...
	if(paginador == 'S' || paginador == 's')
	{
		...
		//var pagini=()
		caux+='<table cellpadding="'+cellP+'" border="'+0+'" bordercolor="'+brcolor+'" cellspacing="'+cellS+'" id="Paginador_'+NomId+'" class="paginador_GridRC">';
		...
		/*caux+='<option value="80">80</option>';
		caux+='<option value="100">100</option>';
		caux+='<option value="150">150</option>';
		caux+='<option value="200">200</option>';
		caux+='<option value="-1">Todos</option>';*/
		caux+='</select>&nbsp;&nbsp;';
		caux+='<td class="prev_paginador_GridRC">';
	//se envia la variable tabla_lst para pintar de verde transferencias ya imprimidas Oscar 25.05.2018		
		caux+='&nbsp;&nbsp;<img class="first_grid" src="'+ruta+'first.png" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="firstPaginador(\''+NomId+'\',\''+tabla_list+'\')">&nbsp;';
	//se envia la variable tabla_lst para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
		caux+='<img class="prev_grid" src="'+ruta+'prev.png" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="antPaginador(\''+NomId+'\',\''+tabla_list+'\')">&nbsp;&nbsp;';
	...
	//se envia la variable tabla_lst para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
		caux+='&nbsp;&nbsp;<img class="next_grid" src="'+ruta+'next.png" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="sigPaginador(\''+NomId+'\',\''+tabla_list+'\')">&nbsp;';
	//se envia la variable tabla_lst para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
		caux+='<img class="last_grid" src="'+ruta+'last.png" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="lastPaginador(\''+NomId+'\',\''+tabla_list+'\')">&nbsp;&nbsp;';
...
function EditarValor(event,celda)
{	
	//alert("entro funcion editar");
	...
	switch(tipo)
	{
		...
		case 'fecha':
			...
			if(tipo == 'entero'){
				extra=extra+' onkeyup="enteros(this);validaEventoTeclado(event,this);"';
			}
			else if(tipo == 'decimal'){
				extra=extra+' onkeyup="decimales(this);validaEventoTeclado(event,this);"';
			}
		...
		case 'eliminador':			
			...
			break;
		case 'combo':			
			//alert(celda.innerHTML.toLowerCase());
...
function DesEditaCelda(cad,obj,val_buscador_global)//cambio implementado por Oscar 20/02/2018 se agrega variable de buscador global
{	/*nota: el objeto es lo mismo que cad!!!*/
	//alert("cad:"+cad+"\nobj:"+$(obj).attr("id")+"\nval_buscador_global"+val_buscador_global);
//obtenenmos la celda
	var celda=document.getElementById(cad);	
	//alert(celda);
//separamos los valores del id de la celda
	var aux=celda.id.split("_");
//nombre de tabla
	var tabla=aux[0];
//posición de la fila (tr)
	var fila=aux[1];
//posición de la columna (td)
	var columna=aux[2];

//obtenemos la cabecera
	var cab=document.getElementById("H_"+tabla+fila);
//obtenemos el tipo de dato referenete a la cabecera
	var tipo=cab.tipo?cab.tipo:cab.getAttribute('tipo');
//obtenemos el valor al cambiar combo si es que es buscador
	var funcam=cab.onChange?cab.onChange:cab.getAttribute('onChange');	
//obtenemos si es o no es modificable la celda
	var modificable=cab.modificable?cab.modificable:cab.getAttribute('modificable');
//obtenemos la máscara (formateo de datos)
	var maskara=cab.mascara?cab.mascara:cab.getAttribute('mascara');
//obtenemos si la columna tiene sumatoria
	var verSum=cab.verSumatoria?cab.verSumatoria:cab.getAttribute('verSumatoria');
//obtenemos si la columna tiene alguna función de validación de datos
	var valida=cab.valida?cab.valida:cab.getAttribute('valida');
	
	//var onChange=cab.onChange?cab.onChange:cab.getAttribute('onChange');
	var nver=false;	
//extraemos el valor real del tr
	var yreal=yReal(cad);
//implementacion de Oscar 20/02/2017 para obtener el objeto (celda)
...
//fin de cambio
	...
	if(!obj){
	//si el objeto no existe paramos la función
		return false;
	}
	...
	switch(tipo)
	{
		...
		case 'fecha':			
			...
			if(verSum == "S")
			{
				...
				if(!isNaN(parseFloat(celda.valor?celda.valor:celda.getAttribute('valor'))) && val > 0)
					val-=parseFloat(celda.valor?celda.valor:celda.getAttribute('valor'));
					
				//alert("val 1:"+val+" resta: "+obj.value);	
	...
	{
		//alert('Se ejecuta onchange');
		var funaux=funcam;
		//alert(funaux);
		
		if(funaux.indexOf('function onchange(event) {'))
		{
			funaux=funaux.replace('function onchange(event) {','');
			funaux=funaux.replace('}','');
		}
...
{
	//alert("especiales");
//Obtenemos el codigo del evento de teclado	
	var key=0;	
	key=(eve.which) ? eve.which : eve.keyCode;	
	
//Separamos los datos del id
	var aux=cad.split('_');
	var tabla=aux[0];//nombre de la tabla
	var columna=parseInt(aux[1]);//numero de fila (tr)
	var fila=parseInt(aux[2]);//numero de columna (td)
//extrtaemos la posición real de la fila
	var yreal=yReal(cad);
...
//Elementos para obtener la fila anterior y siguiente
	if(key != 37 && key != 39){//si la tecla es diferente de izquierda o derecha		
	//Obtenemos la fila actual y conseguimos la siguiente y la anterior
		...
		//conseguimos el numero siguiente
			if(pos == 0){
				var ant=null;
			}else{
...
function ordena(colOrdena,tabla,tabla_list)//se agreega la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018		
{
	...
	if((ordenaPHP == 'S' || ordenaPHP == 's') && (paginador == 'S' || paginador == 's') && pagMetodo == 'php')
	{
		...
		//alert(tabla_list);
		RecargaGrid(tabla,'',tabla_list);//se envia la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018		
...
function InsertaFila(tabla,datos)
{	
//	alert(tabla);
...
//alert(inicial);
		...
		newCell.onclick=function(){EditarValor('no aplica',this);}//la quita Oscar 13/02/2017 esta es la celda que servia como buscador		
		newCell.id=idcelda;
...
{	
	...
	for(j=1;j<lon;j++)
	{
		...
		newCell.height=AltoGral;
		newCell.setAttribute('height', AltoGral);
		newCell.onclick=function(){EditarValor('no aplica',this);}		
		newCell.id=idcelda;
		newCell.setAttribute('id', idcelda);
		newCell.align=alin;	
		...
		if(tipo == 'binario')
		{
			newCell.innerHTML+='<input type="checkbox" value="SI" onBlur="" id="c'+idcelda+'" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');" class="check">';//DesEditaCelda(\''+idcelda+'\', this);
		}
		else if(tipo == 'eliminador')
		{
			newCell.innerHTML='<img src="'+ruta+'borr2.gif" onclick="EliminaFila(\''+idcelda+'\')" id="c'+idcelda+'" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';">';
...
function ocultaBus(val, id)
{
	...
	//alert(id);
	EditarValor('no aplica',document.getElementById(id));	
...
function RecargaGrid(tabla, filar,tabla_list)//se agrega variable tabla list para que al recargar el grid si es transferencias se pinte de verde Oscar 24.05.2018
{
	var lons=new Array();
	var tipos=new Array();
	var alins=new Array();
	...
	//implementación de Oscar para poner transferencias impreas en verde 25.05.2018*/
		...
		/*implementacion Oscar 18.07.2019 para colorear en rojo el listado de remisiones cuando hay diferencia*/
			...
			if(tabla_list=='ZWNfb2NfcmVjZXBjaW9u'){
				...
				if(axF[9]=="Pendiente de Pagar"){//axF[3]==axF[4] && axF[5]==axF[6] && axF[9]!='Pagado'
					color_transfer='style="background:rgba(0,225,0,.5);"';
				}	
			}
		/*fin de cambio Oscar 18.07.2019*/

			NewTabla+='<tr '+color_transfer+' class="NormalCell" height="'+alto+'" onmouseover="selfil(this);" onmouseout="dselfil(this);" class="NormalCell" id="'+tabla+'_Fila'+((k+indk)-1)+'"';

		/*implementación oscar 20.03.2019 para meter el tooltip*/
				if(tabla_list=='ZWNfdHJhbnNmZXJlbmNpYXM='){
					NewTabla+=' title="'+axF[7]+'"';
				}			
				NewTabla+='>';	
		/*Fin de Cambio Oscar 20.03.2019*/
			...
			for(l=0;l<axF.length;l++)
			{
				...
				if(htmldb[l]!="S"&&tipos[l] != 'binario')//MS
					NewTabla+=' onclick="EditarValor(\'no aplica\',this);"';
				if(tipos[l] == 'libre')
				...
				if(tipos[l] == 'binario')//MS
				{
					...
					NewTabla+='<input type="checkbox" value="SI" '+sel+' onBlur="DesEditaCelda(\''+idcelda+'\', this);" id="c'+idcelda+'" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');" valor='+valor+'  class="check" '+extra+'>';
				}////////=***********************************************************************
				else if(tipos[l] == 'libre')//MS
				{							
					if(htmldb[l]!="S")
...
function valorXY(tabla, rpx, py, valor){
	...
	if(Trs.length > 0 && py < Trs.length)
	{
		...
		if(Tds.length > 0 && rpx < Tds.length)
		{
			...
			{				
				obj.value=valor;	
				DesEditaCelda(tabla+'_'+rpx+'_'+py, obj);//desactivado por Osacr 13-02-2017			
			}
			else
			{
...
function Foco(tabla, posx, posy)
{
	...
	if(!obj)
		...
		return EditarValor('no aplica',obj);	
}
...
function sigPaginador(tabla,tabla_list)//se agrega la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
{	
	obj=document.getElementById('numpages_paginador_'+tabla);
	...
	RecargaGrid(tabla, '',tabla_list);//se envía la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
}

function antPaginador(tabla,tabla_list)//se agrega la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
{		
	obj=document.getElementById(tabla);
	var pagAct=obj.pagAct?obj.pagAct:obj.getAttribute('pagAct');	
	pagAct=parseInt(pagAct);
...
function firstPaginador(tabla,tabla_list)//se agrega la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018	
{	
	obj=document.getElementById(tabla);
	var pagAct=obj.pagAct?obj.pagAct:obj.getAttribute('pagAct');	
	pagAct=parseInt(pagAct);
...
function lastPaginador(tabla,tabla_list)//se agrega la variable de la tabla de listado para pintar de verde transferencias ya imprimidas Oscar 25.05.2018		
{	
	obj=document.getElementById('numpages_paginador_'+tabla);
	var numPags=isNaN(parseFloat(obj.innerHTML))?0:parseFloat(obj.innerHTML);
	obj=document.getElementById(tabla);
